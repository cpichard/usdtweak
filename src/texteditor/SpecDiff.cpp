...
void AddDifference(std::vector<std::string> *differences, const SdfPath &path,
                   const std::string &message) {
    if (differences) {
        differences->push_back(path.GetString() + ": " + message);
    }
}
...
bool CompareMetadata(const ParsedMetadata &parsed, const SdfSpec &spec, const SdfPath &path,
                     bool (*isMetadataField)(const TfToken &), std::vector<std::string> *differences,
                     std::vector<std::string> *skippedFields, bool isLayer = false) {
    bool same = true;

    TfTokenVector fields = spec.ListFields();
    for (const TfToken &field : fields) {
        if (!isMetadataField(field) || WriterSkipsField(spec, field, isLayer)) {
            continue;
        }
        if (parsed.unknownFields.count(field)) {
            if (skippedFields) {
                skippedFields->push_back(path.GetString() + ": " + field.GetString());
            }
            continue;
        }
        auto it = parsed.fields.find(field);
        if (it == parsed.fields.end()) {
            AddDifference(differences, path, "metadata '" + field.GetString() + "' missing from parse");
            same = false;
            continue;
        }
        VtValue specValue = spec.GetField(field);
        VtValue parsedValue = it->second;
        if (field == SdfFieldKeys->Relocates && parsedValue.IsHolding<SdfRelocatesMap>()) {
            // the writer relativizes relocate paths to the prim: re-anchor
            SdfRelocatesMap anchored;
            for (const auto &entry : parsedValue.UncheckedGet<SdfRelocatesMap>()) {
                anchored[entry.first.MakeAbsolutePath(path)] = entry.second.MakeAbsolutePath(path);
            }
            parsedValue = VtValue(anchored);
        }
        if (parsedValue != specValue) {
            AddDifference(differences, path,
                          "metadata '" + field.GetString() + "' differs: parsed " +
                              TfStringify(parsedValue) + " vs spec " + TfStringify(specValue));
            same = false;
        }
    }

    // Reverse: parsed fields that the spec does not have
    for (const auto &entry : parsed.fields) {
        if (std::find(fields.begin(), fields.end(), entry.first) == fields.end()) {
            AddDifference(differences, path,
                          "parsed metadata '" + entry.first.GetString() + "' not present on the spec");
            same = false;
        }
    }
    return same;
...
bool ComparePrimBody(const ParsedPrim &parsed, const SdfPrimSpecHandle &spec,
                     std::vector<std::string> *differences, std::vector<std::string> *skippedFields);

bool ComparePrim(const ParsedPrim &parsed, const SdfPrimSpecHandle &spec,
                 std::vector<std::string> *differences, std::vector<std::string> *skippedFields) {
    bool same = true;
    const SdfPath path = spec->GetPath();

    if (parsed.hasSpecifier && parsed.specifier != spec->GetSpecifier()) {
        AddDifference(differences, path, "specifier differs");
        same = false;
    }
    if (parsed.name != spec->GetName()) {
        AddDifference(differences, path, "name differs: parsed '" + parsed.name + "'");
        same = false;
    }

    // Expected type name follows the writer's preamble logic
    std::string expectedTypeName;
    bool expectTypeName = SdfIsDefiningSpecifier(spec->GetSpecifier())
                              ? true
                              : spec->HasField(SdfFieldKeys->TypeName);
    if (expectTypeName) {
        TfToken typeName = spec->GetTypeName();
        if (typeName == SdfTokens->AnyTypeToken) {
            typeName = TfToken();
        }
        expectedTypeName = typeName.GetString();
    }
    const std::string parsedTypeName = parsed.hasTypeName ? parsed.typeName : "";
    if (parsedTypeName != expectedTypeName) {
        AddDifference(differences, path,
                      "type name differs: parsed '" + parsedTypeName + "' vs '" + expectedTypeName + "'");
        same = false;
    }

    same &= CompareMetadata(parsed.metadata, spec.GetSpec(), path, IsPrimMetadataField, differences,
                            skippedFields);
    same &= ComparePrimBody(parsed, spec, differences, skippedFields);
    return same;
}
...
bool ComparePrimBody(const ParsedPrim &parsed, const SdfPrimSpecHandle &spec,
    ...
    // Reorder statements (the writer emits them only when size > 1)
    const std::vector<TfToken> &propertyOrder = spec->GetPropertyOrder();
    const std::vector<TfToken> expectedPropertyOrder =
        propertyOrder.size() > 1 ? propertyOrder : std::vector<TfToken>();
    if (parsed.propertyOrder != expectedPropertyOrder) {
        AddDifference(differences, path, "property order differs");
        same = false;
    }
    const std::vector<TfToken> &childrenOrder = spec->GetNameChildrenOrder();
    const std::vector<TfToken> expectedChildrenOrder =
        childrenOrder.size() > 1 ? childrenOrder : std::vector<TfToken>();
    if (parsed.nameChildrenOrder != expectedChildrenOrder) {
        AddDifference(differences, path, "name children order differs");
        same = false;
    }

    // Properties: every spec property must be parsed and match
    const SdfPropertySpecView properties = spec->GetProperties();
    if (parsed.properties.size() != properties.size()) {
        AddDifference(differences, path,
                      TfStringPrintf("property count differs: parsed %zu vs spec %zu",
                                     parsed.properties.size(), properties.size()));
        same = false;
    }
    for (const ParsedProperty &parsedProperty : parsed.properties) {
        SdfPropertySpecHandle propertySpec = properties.get(TfToken(parsedProperty.name));
        if (!propertySpec) {
            AddDifference(differences, path, "parsed property '" + parsedProperty.name +
                                                 "' not present on the spec");
            same = false;
            continue;
        }
        same &= CompareParsedPropertyToSpec(parsedProperty, propertySpec, differences, skippedFields);
    }

    // Children
    const SdfPrimSpecView children = spec->GetNameChildren();
    if (parsed.children.size() != children.size()) {
        AddDifference(differences, path,
                      TfStringPrintf("child count differs: parsed %zu vs spec %zu",
                                     parsed.children.size(), children.size()));
        same = false;
    }
    for (const auto &parsedChild : parsed.children) {
        SdfPrimSpecHandle childSpec = children.get(TfToken(parsedChild->name));
        if (!childSpec) {
            AddDifference(differences, path,
                          "parsed child '" + parsedChild->name + "' not present on the spec");
            same = false;
            continue;
        }
        same &= ComparePrim(*parsedChild, childSpec, differences, skippedFields);
    }

    // Variant sets (the writer skips sets with no variants)
    size_t expectedVariantSets = 0;
    const SdfVariantSetsProxy variantSets = spec->GetVariantSets();
    for (const auto &nameAndSet : variantSets) {
        if (!nameAndSet.second->GetVariantList().empty()) {
            ++expectedVariantSets;
        }
    }
    if (parsed.variantSets.size() != expectedVariantSets) {
        AddDifference(differences, path, "variant set count differs");
        same = false;
    }
    for (const ParsedVariantSet &parsedSet : parsed.variantSets) {
        ...
        if (setIt == variantSets.end()) {
            AddDifference(differences, path,
                          "parsed variant set '" + parsedSet.name + "' not present on the spec");
            same = false;
            continue;
        }
        const SdfVariantSetSpecHandle &setSpec = setIt->second;
        const SdfVariantSpecHandleVector variants = setSpec->GetVariantList();
        if (parsedSet.variants.size() != variants.size()) {
            AddDifference(differences, path,
                          "variant count differs in set '" + parsedSet.name + "'");
            same = false;
        }
        for (const ParsedVariant &parsedVariant : parsedSet.variants) {
            SdfVariantSpecHandle variantSpec;
            for (const SdfVariantSpecHandle &candidate : variants) {
            ...
            if (!variantSpec) {
                AddDifference(differences, path, "parsed variant '" + parsedVariant.name +
                                                     "' not present in set '" + parsedSet.name + "'");
                same = false;
                continue;
            }
            const SdfPrimSpecHandle variantPrim = variantSpec->GetPrimSpec();
            same &= CompareMetadata(parsedVariant.body->metadata, variantPrim.GetSpec(),
                                    variantPrim->GetPath(), IsPrimMetadataField, differences,
                                    skippedFields);
            same &= ComparePrimBody(*parsedVariant.body, variantPrim, differences, skippedFields);
        }
    }
    return same;
}
...
bool CompareParsedLayerToLayer(const ParsedLayer &parsed, const SdfLayerRefPtr &layer,
    ...
    same &= CompareMetadata(parsed.metadata, pseudoRoot.GetSpec(), SdfPath::AbsoluteRootPath(),
                            IsLayerMetadataField, differences, skippedFields, /* isLayer = */ true);

    const std::vector<TfToken> &rootOrder = layer->GetRootPrimOrder();
    const std::vector<TfToken> expectedOrder =
        rootOrder.size() > 1 ? rootOrder : std::vector<TfToken>();
    if (parsed.rootPrimOrder != expectedOrder) {
        AddDifference(differences, SdfPath::AbsoluteRootPath(), "root prim order differs");
        same = false;
    }

    if (parsed.rootPrims.size() != layer->GetRootPrims().size()) {
        AddDifference(differences, SdfPath::AbsoluteRootPath(), "root prim count differs");
        same = false;
    }
    for (const auto &parsedPrim : parsed.rootPrims) {
        SdfPrimSpecHandle spec = layer->GetRootPrims().get(TfToken(parsedPrim->name));
        if (!spec) {
            AddDifference(differences, SdfPath::AbsoluteRootPath(),
                          "parsed root prim '" + parsedPrim->name + "' not present on the layer");
            same = false;
            continue;
        }
        same &= ComparePrim(*parsedPrim, spec, differences, skippedFields);
    }
    return same;
}
...
bool CompareParsedPrimToSpec(const ParsedPrim &parsed, const SdfPrimSpecHandle &spec,
                             ...
                             std::vector<std::string> *skippedFields) {
    if (!spec) {
        AddDifference(differences, SdfPath(), "no spec to compare against");
        return false;
    }
    return ComparePrim(parsed, spec, differences, skippedFields);
}
...
bool CompareParsedPropertyToSpec(const ParsedProperty &parsed, const SdfPropertySpecHandle &spec,
                                 ...
                                 std::vector<std::string> *skippedFields) {
    if (!spec) {
        AddDifference(differences, SdfPath(), "no property spec to compare against");
        return false;
    }
    bool same = true;
    const SdfPath path = spec->GetPath();
    const bool specIsRelationship = (spec->GetSpecType() == SdfSpecTypeRelationship);

    if (parsed.isRelationship != specIsRelationship) {
        AddDifference(differences, path, "attribute/relationship kind differs");
        return false;
    }
    if (parsed.custom != spec->IsCustom()) {
        AddDifference(differences, path, "custom flag differs");
        same = false;
    }
    if (parsed.variability != spec->GetField(SdfFieldKeys->Variability)
                                  .GetWithDefault<SdfVariability>(
                                      specIsRelationship ? SdfVariabilityUniform : SdfVariabilityVarying)) {
        AddDifference(differences, path, "variability differs");
        same = false;
    }

    if (!specIsRelationship) {
        const SdfAttributeSpecHandle attribute = SdfSpecStatic_cast<SdfAttributeSpecHandle>(spec);
        const std::string specTypeName =
            SdfValueTypeNames->GetSerializationName(attribute->GetTypeName()).GetString();
        if (parsed.typeName != specTypeName) {
            AddDifference(differences, path, "attribute type differs: parsed '" + parsed.typeName +
                                                 "' vs '" + specTypeName + "'");
            same = false;
        }
    }
    ...
    // Default value
    if (parsed.unsupportedValueType && skippedFields) {
        skippedFields->push_back(path.GetString() + ": values (unregistered type '" +
                                 parsed.typeName + "')");
    }
    const VtValue specDefault = spec->GetField(SdfFieldKeys->Default);
    const bool expectDefault = !specDefault.IsEmpty();
    if (parsed.hasDefault != expectDefault) {
        AddDifference(differences, path, parsed.hasDefault ? "parsed default not on the spec"
                                                           : "spec default missing from parse");
        same = false;
    } else if (expectDefault && !parsed.foldedDefault && !parsed.unsupportedValueType &&
               parsed.defaultValue != specDefault) {
        AddDifference(differences, path,
                      "default differs: parsed " + TfStringify(parsed.defaultValue) + " vs spec " +
                          TfStringify(specDefault));
        same = false;
    }

    // Connections / targets
    const TfToken listField = specIsRelationship ? SdfFieldKeys->TargetPaths
                                                 : SdfFieldKeys->ConnectionPaths;
    const VtValue specListValue = spec->GetField(listField);
    const bool parsedHasList = specIsRelationship ? parsed.hasTargets : parsed.hasConnections;
    const SdfPathListOp &parsedList = specIsRelationship ? parsed.targets : parsed.connections;
    if (parsedHasList != !specListValue.IsEmpty()) {
        AddDifference(differences, path,
                      specIsRelationship ? "relationship targets presence differs"
                                         : "connections presence differs");
        same = false;
    } else if (parsedHasList && specListValue.IsHolding<SdfPathListOp>() &&
               parsedList != specListValue.UncheckedGet<SdfPathListOp>()) {
        AddDifference(differences, path, specIsRelationship ? "relationship targets differ"
                                                            : "connections differ");
        same = false;
    }

    // Time samples
    const VtValue specSamplesValue = spec->GetField(SdfFieldKeys->TimeSamples);
    if (parsed.hasTimeSamples != specSamplesValue.IsHolding<SdfTimeSampleMap>()) {
        AddDifference(differences, path, "timeSamples presence differs");
        same = false;
    } else if (parsed.hasTimeSamples) {
        const SdfTimeSampleMap &specSamples = specSamplesValue.UncheckedGet<SdfTimeSampleMap>();
        if (specSamples.size() != parsed.timeSamples.size()) {
            AddDifference(differences, path, "timeSamples count differs");
            same = false;
        } else {
            for (const auto &sample : parsed.timeSamples) {
                auto it = specSamples.find(sample.first);
                if (it == specSamples.end()) {
                    AddDifference(differences, path,
                                  TfStringPrintf("time sample %f missing on the spec", sample.first));
                    same = false;
                    continue;
                }
                const bool folded =
                    std::find(parsed.foldedSampleTimes.begin(), parsed.foldedSampleTimes.end(),
                              sample.first) != parsed.foldedSampleTimes.end();
                if (!folded && sample.second != it->second) {
                    AddDifference(differences, path,
                                  TfStringPrintf("time sample %f differs", sample.first));
                    same = false;
                }
            }
        }
    ...
    // Splines are opaque: presence compared, content skipped
    const VtValue specSpline = spec->GetField(SdfFieldKeys->Spline);
    if (parsed.hasSpline != !specSpline.IsEmpty()) {
        AddDifference(differences, path, "spline presence differs");
        same = false;
    } else if (parsed.hasSpline && skippedFields) {
        skippedFields->push_back(path.GetString() + ": spline content (opaque)");
    }

    same &= CompareMetadata(parsed.metadata, spec.GetSpec(), path,
                            specIsRelationship ? IsRelationshipMetadataField : IsAttributeMetadataField,
                            differences, skippedFields);
    return same;
}
