...
ToolDefs BuildReadOnlyToolDefinitions() {
    ...
    // 7b. run_query — UTQL, the expressive complement to find_prims.
    {
        ...
        props["query"] = MakeStringParam(
            ...
            "  FIND <entity> [COMPOSED FROM <origin> | CONNECTED "
            "[UPSTREAM|DOWNSTREAM] (TO|OF) <origin> [WITHIN n]] "
            "[IN <scope>] [AT <time>] [WHERE <cond>] "
            "[RETURN <fields>|*] [ORDERED BY <field> [ASC|DESC]] [LIMIT n] "
            "[AS \"name\"]\n"
            "  COMPOSED FROM <origin> = LAYER \"id\" PATH \"/p\" | \"/path\" | "
            "RESULTSET \"name\".  <scope> = STAGE \"id\" | STAGES \"*\" | "
            ...
            "ENTITIES (this tool): USDPRIM, USDATTRIBUTE, USDRELATIONSHIP "
            "(composed/Stage) and LAYER (the stage's layers + their metadata). "
            "The authored per-spec entities SDFPRIM/SDFATTRIBUTE/SDFRELATIONSHIP "
            "and COMPOSING INTO are NOT supported here — use find_prims / the "
            "get_* tools instead.\n"
            ...
            "(stage scope) to bound the search. The inverse of COMPOSING INTO (which "
        ...
        tools.push_back(JsValue(_Tool(
            ...
            "(VALUE.SCALAR), connection reachability (CONNECTED TO), forward "
            "composition (COMPOSED FROM: which composed prims come from / are "
            "defined by a given layer + spec path), instancing "
            ...
            "call again. Composed Stage entities and FIND LAYER only (not authored "
            "SDF*).\n"
