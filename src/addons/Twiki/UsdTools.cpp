...
ToolDefs BuildReadOnlyToolDefinitions() {
    ...
    // 7b. run_query — UTQL, the expressive complement to find_prims.
    {
        ...
        props["query"] = MakeStringParam(
            ...
            "ISINPROTOTYPE• ISINSTANCEPROXY• INSTANCEABLE• ISLOADED•(payload "
            "load state — pair with HAS_PAYLOAD: `HAS_PAYLOAD AND NOT ISLOADED` = "
            "unloaded payloads) "
            ...
            "LAYER fields (one row per layer the stage uses): LAYER.IDENTIFIER "
            "LAYER.DISPLAYNAME LAYER.REALPATH LAYER.FILEFORMAT(\"usda\"/\"usdc\") "
            "LAYER.DIRTY• LAYER.ANONYMOUS• LAYER.MUTED• LAYER.EMPTY• "
            "LAYER.ISROOTLAYER• LAYER.ISSESSIONLAYER• LAYER.DEFAULTPRIM "
            "LAYER.UPAXIS LAYER.METERSPERUNIT LAYER.ROOTPRIMCOUNT LAYER.STARTTIME "
            "LAYER.ENDTIME LAYER.TIMECODESPERSECOND LAYER.FRAMESPERSECOND "
            ...
            "default prim are root-authoritative — gate with LAYER.ISROOTLAYER), "
            ...
            "answer these with a WHERE PATH/PRIMNAME filter. Origin forms: LAYER "
            ...
            "  FIND USDPRIM WHERE HAS_PAYLOAD AND NOT ISLOADED\n"
            ...
            "LAYER.IDENTIFIER, SUBLAYERS\n"
            "  FIND LAYER WHERE LAYER.DIRTY\n"
