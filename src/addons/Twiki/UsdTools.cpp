...
ToolDefs BuildReadOnlyToolDefinitions() {
    ...
    // 7b. run_query — UTQL, the expressive complement to find_prims.
    {
        ...
        props["query"] = MakeStringParam(
            ...
            "[AS \"name\"]\n"
            "  COMPOSED FROM <origin> = LAYER \"id\" PATH \"/p\" | \"/path\" | "
            ...
            "USDPRIM, USDATTRIBUTE, USDRELATIONSHIP — the resolved scene after "
            "composition. AUTHORED (Layer world, per-spec): SDFPRIM, SDFATTRIBUTE, "
            "SDFRELATIONSHIP — raw opinions as written in each layer, before "
            "composition (one row per spec, so the same prim/attr appears once per "
            "layer that authors it). Plus LAYER (the stage's layers + metadata). "
            "Pick USD* for 'what the prim ends up being'; pick SDF* for 'what THIS "
            "layer authors / where an opinion is written'. The full UTQL surface "
            "runs here, both composition directions included (COMPOSED FROM and "
            "COMPOSING INTO).\n"
            "WHERE operators: `field OP literal` (OP = = != < <= > >=); "
            "`field LIKE \"sub\"` (substring) or `LIKE /regex/`; "
            "`field IN (\"a\",\"b\")`; `set CONTAINS \"v\"` (set/family fields); "
            "`field IS [NOT] NULL`; `TYPE IS_A \"SchemaType\"` (schema-registry "
            "inheritance, prim entities only — TYPE IS_A \"Gprim\" matches "
            "Mesh/Sphere/Cube/…; use for 'all geometry'/'all lights' instead of "
            "enumerating concrete types; abstract names like Gprim/Xformable/"
            "Imageable/Boundable work); "
            "`PATH UNDER \"/p\"` (true namespace prefix); "
            "bare boolflag (e.g. ACTIVE). Combine with AND/OR/NOT and parens "
            "(NOT > AND > OR). Same-family predicates in one AND correlate to ONE "
            "arc.\n"
            "PRIM fields: NAME PATH PARENT(parent prim path; \"/\" for root "
            "prims) TYPE(\"Mesh\") KIND(\"component\") "
            "SPECIFIER(\"def\"/\"over\"/\"class\") ACTIVE• DEPTH CHILD_COUNT "
            "ATTRIBUTE_COUNT HAS_REFERENCE• HAS_PAYLOAD• HAS_VARIANT• "
            "HAS_API• HAS_TIME_SAMPLES• HAS_SPLINE•(any attr has a USD "
            "animation-curve spline — a separate value source from time "
            "samples) HAS_CLIPS•(authored value-clips metadata; 'animated at "
            "all' = HAS_TIME_SAMPLES OR HAS_SPLINE OR HAS_CLIPS) "
            "IS_INSTANCE• IS_PROTOTYPE• "
            "IS_IN_PROTOTYPE• IS_INSTANCE_PROXY• INSTANCEABLE• IS_LOADED•(payload "
            "load state — pair with HAS_PAYLOAD: `HAS_PAYLOAD AND NOT IS_LOADED` = "
            "unloaded payloads) "
            "HAS_RELATIONSHIP• RELATIONSHIPS(set: CONTAINS \"material:binding\") "
            "IS_IN_VARIANT•/VARIANT_SELECTIONS(set: CONTAINS \"{look=red}\"; SDF* "
            "only — the variant a spec is authored inside) "
            "HAS_ASSETINFO• ASSETINFO.IDENTIFIER ASSETINFO.NAME ASSETINFO.VERSION "
            "ASSETINFO.DEPENDENCIES(set: CONTAINS \"dep.usd\") (assetInfo metadata; "
            "IDENTIFIER = authored asset-path string, absent keys are NULL — "
            "`ASSETINFO.VERSION IS NULL`) "
            "HAS_CUSTOMDATA• CUSTOMDATA.KEYS(set of colon-joined key paths: "
            "CONTAINS \"pipeline:priority\") CUSTOMDATA[\"key\"] (one customData "
            "entry — key is case-sensitive, colon nests into sub-dicts: "
            "CUSTOMDATA[\"pipeline:reviewState\"] = \"approved\"; missing key = "
            "NULL so IS NOT NULL tests existence; value is type-polymorphic like "
            "VALUE.SCALAR; authored data only, schema fallbacks excluded); "
            "METADATA[\"key\"] (any registered scalar metadatum by name — prim, "
            "attribute, relationship — e.g. METADATA[\"documentation\"], "
            "METADATA[\"hidden\"], METADATA[\"comment\"]; authored-only, missing = "
            "NULL; keys that have a dedicated field like kind/active/customData are "
            "an error naming that field); "
            "families: REFERENCE.* / PAYLOAD.* (.ASSET .PRIM_PATH .IS_MISSING• "
            "(broken/unresolved asset — pair with HAS_REFERENCE/HAS_PAYLOAD) "
            ".LAYER_OFFSET .LAYER_SCALE), VARIANT.* (.SET/.SELECTION), and "
            "API (API CONTAINS \"PhysicsCollisionAPI\", API.COUNT).\n"
            "RETURN of an arc field shows ONLY the arcs matched by a same-family "
            "WHERE filter (the witness): WHERE REFERENCE.IS_MISSING RETURN "
            "REFERENCE.ASSET lists just the missing reference, not every reference. "
            "No same-family filter (or a negated one, NOT REFERENCE.IS_MISSING) "
            "joins ALL arcs; narrowing is per-family.\n"
            "ATTRIBUTE fields: NAME(full name incl. namespace: "
            "\"inputs:intensity\") BASENAME(after the last colon: "
            "\"intensity\" — use BASENAME = \"x\" when unsure whether the "
            "attr is namespaced) TYPE "
            "NAMESPACE(\"primvars\") PARENT(owning prim path) "
            "VALUE.SCALAR (resolved value; "
            "numeric/string/bool, arrays excluded so gate with NOT VALUE.IS_ARRAY) "
            "VALUE.IS_ARRAY• VALUE.ARRAY_SIZE VALUE.HAS_TIME_SAMPLES• "
            "VALUE.HAS_SPLINE•(authored animation-curve spline; disjoint from "
            "time samples) VALUE.IS_BLOCKED•(value is an authored block or "
            "unreadable) VARIABILITY INTERPOLATION "
            "HAS_CONNECTION• CONNECTION.SOURCE(set) CONNECTION.COUNT "
            "IS_IN_VARIANT•/VARIANT_SELECTIONS(set; SDF* only) "
            "ASSET.IS_MISSING• (asset/asset[] value doesn't resolve) PATH. Use "
            "AT TIME t to pin a frame; otherwise the UI's current time is used. "
            "(• = unary bool flag.)\n"
            "RELATIONSHIP fields: NAME NAMESPACE BASENAME PARENT(owning prim "
            "path) "
            "TARGET(set of target paths: CONTAINS \"/Looks/Metal\") TARGET_COUNT "
            "TARGET.IS_MISSING•(dangling target — some target path resolves to no "
            "object on the stage; USDRELATIONSHIP only, empty target lists don't "
            "match).\n"
            "SDF* fields mirror their USD* counterparts (SDFPRIM≈USDPRIM, "
            "SDFATTRIBUTE≈USDATTRIBUTE, SDFRELATIONSHIP≈USDRELATIONSHIP) but read "
            "AUTHORED specs, so composed-only gates do NOT apply: IS_INSTANCE / "
            "IS_PROTOTYPE / IS_IN_PROTOTYPE / IS_INSTANCE_PROXY / IS_LOADED / "
            "TARGET.IS_MISSING are "
            "Stage-only (use INSTANCEABLE for the authored metadata flag). "
            "SDFATTRIBUTE VALUE.SCALAR reads the authored default in that layer "
            "(no composition); SPECIFIER tells over/def/class per spec — "
            "`SDFPRIM WHERE SPECIFIER = \"over\"` finds override opinions. "
            "SDFPRIM/SDFATTRIBUTE also have SDF-only variant-nesting fields "
            "(inverse of the Stage-only gates): IS_IN_VARIANT• (the spec is "
            "authored inside a variant scope) and VARIANT_SELECTIONS (set of the "
            "\"{set=value}\" scopes it is nested under: CONTAINS \"{look=red}\"). "
            "These describe the variant a spec lives INSIDE — distinct from the "
            "VARIANT.* family (variant sets defined ON a prim). SDF* "
            "scans every layer the stage uses by default; narrow with IN "
            "LAYERSTACK, IN SUBLAYERS, or IN LAYER \"id\".\n"
            "LAYER fields (one row per layer the stage uses): IDENTIFIER "
            "DISPLAY_NAME REAL_PATH FILE_FORMAT(\"usda\"/\"usdc\") "
            "DIRTY• ANONYMOUS• MUTED• EMPTY• "
            "IS_ROOT_LAYER• IS_SESSION_LAYER• DEFAULT_PRIM "
            "UP_AXIS METERS_PER_UNIT ROOT_PRIM_COUNT START_TIME "
            "END_TIME TIMECODES_PER_SECOND FRAMES_PER_SECOND "
            "HAS_SUBLAYER• SUBLAYER.COUNT and the SUBLAYER family (one arc per "
            "sublayer, existential/correlated like REFERENCE): SUBLAYER.ASSET(authored "
            "path: SUBLAYER.ASSET LIKE \"layout.usda\" or CONTAINS \"./layout.usda\") "
            "SUBLAYER.IS_MISSING•(authored sublayer that does not resolve) "
            "SUBLAYER.LAYER_OFFSET. Use FIND LAYER for 'which layers sublayer X', "
            "'which layers have a missing/broken sublayer' (SUBLAYER.IS_MISSING), "
            "stage settings (up-axis/fps/"
            "default prim are root-authoritative — gate with IS_ROOT_LAYER), "
            "dirty/anonymous/muted layer audits. Scope it with IN STAGE \"id\", "
            "IN LAYERSTACK, or IN SUBLAYERS; default = all layers the stage uses. "
            "AT / CONNECTED TO / COMPOSING INTO are NOT valid on LAYER.\n"
            ...
            "COMPOSED FROM (USD* only) is forward composition. USE IT whenever the "
            "question is 'which prims are composed from / come from / are defined by "
            "/ pulled in from <a layer and/or a spec path>' — e.g. 'usdprims "
            "composed from layer.usda /Root/Object' → `FIND USDPRIM COMPOSED FROM "
            "LAYER \"layer.usda\" PATH \"/Root/Object\"`. It returns the composed "
            "prims an authored spec FEEDS, following the reference/variant path remap "
            "that a `WHERE PATH = \"/Root/Object\"` lookup would MISS — so do NOT "
            "answer these with a WHERE PATH/NAME filter. Origin forms: LAYER "
            "\"id\" PATH \"/p\" (a layer + a spec path), a bare \"/path\" (that spec "
            "in any layer), or RESULTSET \"name\" (a Layer-world set). Keeps IN "
            "(stage scope) to bound the search. The inverse of COMPOSING INTO.\n"
            "COMPOSING INTO (SDF* only) is the reverse — composition INVERSION. USE "
            "IT for 'which authored specs / opinions COMPOSE INTO / contribute to / "
            "build <a composed prim path>' — e.g. 'which layers author /World/Hero' "
            "→ `FIND SDFPRIM COMPOSING INTO \"/World/Hero\"`, one row per spec in the "
            "prim's composed stack (strongest first). Target = a composed \"/path\", "
            "a list (\"/a\",\"/b\"), or RESULTSET \"name\"; it REPLACES the IN clause. "
            "Add PER TARGET to also expose COMPOSITION.TARGET / .STRENGTH / .ARC_TYPE "
            "per contributing spec (otherwise specs are de-duplicated across "
            "targets).\n"
            ...
            "world — cached sets here are Stage-world (USDPRIM/…), so reuse them "
            "from Stage queries, not from FIND LAYER.\n"
            ...
            "EXAMPLES:\n"
            "  FIND USDPRIM WHERE TYPE = \"Mesh\" AND NOT ACTIVE\n"
            "  FIND USDPRIM WHERE TYPE IS_A \"Gprim\"\n"
            "  FIND USDPRIM WHERE API CONTAINS \"MaterialBindingAPI\"\n"
            "  FIND USDPRIM WHERE IS_INSTANCE\n"
            "  FIND USDPRIM WHERE HAS_PAYLOAD AND NOT IS_LOADED\n"
            "  FIND USDPRIM WHERE HAS_SPLINE OR HAS_TIME_SAMPLES OR HAS_CLIPS\n"
            "  FIND USDPRIM WHERE HAS_REFERENCE AND REFERENCE.IS_MISSING\n"
            "  FIND USDPRIM WHERE ASSETINFO.VERSION = \"2\" RETURN PATH, "
            "ASSETINFO.IDENTIFIER\n"
            "  FIND USDPRIM WHERE CUSTOMDATA[\"pipeline:reviewState\"] = "
            "\"approved\" RETURN PATH, CUSTOMDATA[\"pipeline:priority\"]\n"
            "  FIND USDRELATIONSHIP WHERE TARGET.IS_MISSING RETURN PATH, TARGET\n"
            "  FIND USDATTRIBUTE WHERE NAME = \"inputs:intensity\" AND "
            "VALUE.SCALAR > 1000\n"
            "  FIND USDATTRIBUTE WHERE ASSET.IS_MISSING\n"
            "  FIND SDFPRIM WHERE SPECIFIER = \"over\"\n"
            "  FIND SDFATTRIBUTE WHERE NAME = \"radius\" IN LAYER \"asset.usda\"\n"
            "  FIND USDPRIM CONNECTED UPSTREAM OF \"/Looks/Mat.outputs:surface\"\n"
            "  FIND USDPRIM COMPOSED FROM LAYER \"modeling.usda\" PATH "
            "\"/root/chair\"\n"
            "  FIND SDFPRIM COMPOSING INTO \"/World/Hero\"\n"
            "  FIND LAYER WHERE SUBLAYER.ASSET LIKE \"layout.usda\" RETURN "
            "IDENTIFIER, SUBLAYER.ASSET\n"
            "  FIND LAYER WHERE SUBLAYER.IS_MISSING RETURN IDENTIFIER, "
            "SUBLAYER.ASSET\n"
            "  FIND LAYER WHERE DIRTY\n"
        ...
        props["store_as"] = MakeStringParam(
            ...
            "and FIND LAYER rows carry no prim path — neither is directly usable "
            "by the prim edit tools.)");
        tools.push_back(JsValue(_Tool(
            "run_query",
            ...
            "(VALUE.SCALAR), connection reachability (CONNECTED TO), composition in "
            "either direction (COMPOSED FROM: which composed prims come from a layer "
            "+ spec path; COMPOSING INTO: which authored specs build a composed "
            "prim), authored per-layer opinions (SDFPRIM/SDFATTRIBUTE), instancing "
            "(IS_INSTANCE / IS_PROTOTYPE), relationship names (RELATIONSHIPS), "
            "animation (HAS_TIME_SAMPLES), missing assets (ASSET.IS_MISSING), "
            "composition predicates (REFERENCE.* / API), boolean combinations of "
            "these, or layer-level questions (FIND LAYER: which layers sublayer a "
            "file, stage settings, dirty/anonymous layers). Prefer find_prims for "
            "simple type/kind/name lookups. The result reports matched/scanned "
            "counts and is capped at 50 displayed rows plus the global 8 KB cap. A "
            "compile error is recoverable — read the message, fix the query, and "
            "call again. The full UTQL surface runs: composed Stage entities (USD*), "
            "authored per-spec entities (SDF*), FIND LAYER, and both composition "
            "directions (COMPOSED FROM / COMPOSING INTO).\n"
            ...
            "run_query is strictly READ-ONLY. To then EDIT what a query can "
...
ToolDefs BuildEditToolDefinitions() {
    ...
    // 19. run_mutation — UTQL write statements, the write complement to run_query.
    {
        JsObject props;
        props["statement"] = MakeStringParam(
            ...
            "[INSIDE VARIANT \"{set=sel}\"] <mutation>+ "
            "[RETURN <fields>] [LIMIT n]\n"
            "  CREATE USDPRIM \"/path\" [TYPE \"Mesh\"] [ON LAYER \"id\"]\n"
            "  CREATE SDFPRIM \"/path\" IN LAYER \"id\" "
            "[SPECIFIER \"def\"|\"over\"|\"class\"] [TYPE \"Mesh\"]\n"
            "  DELETE SDFPRIM|SDFATTRIBUTE|SDFRELATIONSHIP "
            "(COMPOSING INTO <target> | [IN <scope>] WHERE <cond>)\n"
            "  <mutation> = SET <field> = <literal>[, <field> = <literal>…] | "
            "SET VALUE = SAMPLES {t: <literal>, …} | "
            "SET VARIANT[\"set\"] = \"sel\"|NULL | "
            "SET CUSTOMDATA[\"key\"] = <scalar>|NULL | "
            "SET CUSTOMDATA = {\"key\": <scalar>, …}|NULL | "
            "SET METADATA[\"key\"] = <scalar>|NULL | "
            "SET ASSETINFO.NAME|.VERSION|.IDENTIFIER = \"s\"|NULL | "
            "ADD <family> \"value\" [PRIM_PATH \"/p\"] | "
            "ADD VARIANT[\"set\"] \"name\" | REMOVE <family> "
            "[\"value\"] | CREATE ATTRIBUTE \"name\" TYPE \"float\" "
            "[VALUE <literal>] [INTERPOLATION \"constant\"] | "
            "CREATE RELATIONSHIP \"name\" [TARGET \"/p\"]\n"
            "Entities, <scope>, AT and WHERE are exactly run_query's FIND "
            "surface — same fields, operators, and RESULTSET references.\n"
            "SELECTION IS MANDATORY on UPDATE/DELETE: a WHERE, an IN RESULTSET "
            "\"n\", or a named IN LAYER \"id\" scope. Broad scopes alone (no "
            "IN, IN LAYERSTACK, IN SUBLAYERS) are a compile error; a deliberate "
            "whole-scene write must be spelled WHERE PATH UNDER \"/\".\n"
            "DESTINATIONS: USD* (composed) statements write to the stage's "
            "edit target; ON LAYER \"id\" retargets to another layer of the "
            "layer stack. SDF*/LAYER (authored) statements edit the spec's own "
            "layer in place — the IN scope IS the destination and ON LAYER is "
            "an error there.\n"
            "WRITABLE fields — prim: ACTIVE INSTANCEABLE KIND TYPE "
            "VARIANT[\"set\"] CUSTOMDATA[\"key\"] CUSTOMDATA(whole-dict) "
            "ASSETINFO.NAME/.VERSION/.IDENTIFIER METADATA[\"key\"] "
            "(SPECIFIER, NAME, PARENT: SDFPRIM only); "
            "attribute: VALUE INTERPOLATION METADATA[\"key\"] "
            "(VARIABILITY, NAME: SDFATTRIBUTE only); "
            "relationship: METADATA[\"key\"] (targets via ADD/REMOVE TARGET); "
            "layer: DEFAULT_PRIM UP_AXIS START_TIME END_TIME "
            "TIMECODES_PER_SECOND FRAMES_PER_SECOND METERS_PER_UNIT MUTED. "
            "Everything else is read-only.\n"
            "RENAME/REPARENT (SDF* entities ONLY — authored specs, never "
            "USD*): SET NAME = \"newName\" renames the matched prim / "
            "property in place; SET PARENT = \"/new/parent\" (prims only) "
            "moves it under an existing prim, keeping the name (\"/\" makes "
            "it a root prim); SET PARENT = …, NAME = … does both as ONE "
            "namespace edit. PATH is never writable. Cannot be combined with "
            "any other SET/ADD/REMOVE/CREATE clause in one statement. The "
            "edit fixes paths (incl. relationship targets and connections) "
            "within the destination layer only — opinions in OTHER layers "
            "still point at the old path; per-row failures (name collision, "
            "missing destination parent, row inside a variant) are counted "
            "skips with the reason in warnings.\n"
            "SET CUSTOMDATA[\"key\"] = <scalar>|NULL writes one customData "
            "entry per key (comma-batch several in one SET); colon key paths "
            "nest into sub-dicts and intermediates are auto-created "
            "(CUSTOMDATA[\"pipeline:reviewState\"] = \"approved\"); NULL "
            "erases the entry. A number without a decimal point authors int, "
            "with one authors double (7 vs 7.0). "
            "SET CUSTOMDATA = {\"assetType\": \"prop\", \"pipeline:priority\": 3} "
            "REPLACES the whole authored dict (per-key form merges); "
            "SET CUSTOMDATA = NULL clears it.\n"
            "SET METADATA[\"key\"] = <scalar>|NULL writes any registered scalar "
            "metadatum by name on prim/attribute/relationship "
            "(METADATA[\"documentation\"] = \"...\", METADATA[\"hidden\"] = true); "
            "the value is coerced to the registered type (a mismatch is a per-row "
            "skip); NULL clears the opinion. Keys with a dedicated field "
            "(kind/active/typeName/customData/assetInfo/…) are a compile error "
            "naming that field — use the field, not METADATA.\n"
            "SET ASSETINFO.NAME/.VERSION/.IDENTIFIER = \"s\"|NULL writes the model "
            "assetInfo scalars (IDENTIFIER is an asset path); DEPENDENCIES is "
            "read-only.\n"
            "SET VALUE literals: number, \"string\", true/false, tuple "
            "(1, 0, 0) for vec/color (quat = (x,y,z,w) w-last), whole array "
            "[(0,0,0), (1,0,0)] / [1, 2.5, 3] / [] — a bare scalar on an "
            "array-typed attribute is a per-row skip, wrap it in [ … ]. NULL "
            "clears the authored opinion; BLOCK authors a value block. "
            "Literals are coerced to the attribute's type; a row that cannot "
            "coerce is a counted skip, not an error. AT TIME t authors a time "
            "sample at t; with no AT the DEFAULT value is written (reads "
            "default to the current frame, writes deliberately do not).\n"
            "BATCHED KEYFRAMES: SET VALUE = SAMPLES {1: 0.0, 12: 4.5, 24: "
            "9.0} authors ALL the samples in ONE statement/undo entry — "
            "always prefer this over one call per keyframe. Entry values "
            "take anything SET VALUE takes (numbers, tuples, whole arrays); "
            "{t: NULL} erases the sample at t, {t: BLOCK} blocks it. Times "
            "may be negative/fractional. One bad entry skips that row's "
            "WHOLE batch (no partial animations; the warning names the "
            "time). SAMPLES cannot combine with the statement's AT TIME "
            "(the map carries its own times) and never touches the DEFAULT "
            "value.\n"
            "ADD/REMOVE families: REFERENCE PAYLOAD INHERIT SPECIALIZE API on "
            "prim entities (ADD REFERENCE/PAYLOAD take \"asset.usd\" "
            "[PRIM_PATH \"/p\"]; INHERIT/SPECIALIZE take a \"/path\"), "
            "SUBLAYER on LAYER, TARGET on relationships, CONNECTION on "
            "attributes. Bare REMOVE <family> removes ALL the row's arcs "
            "UNLESS the WHERE has a positive same-family predicate — then only "
            "the matched arcs go (WHERE REFERENCE.IS_MISSING REMOVE REFERENCE "
            "deletes just the broken ones; WHERE TARGET.IS_MISSING REMOVE "
            "TARGET strips only the dangling relationship targets). "
            "REMOVE <family> \"value\" removes "
            "that exact entry. Mutation clauses apply in order per row, so "
            "REMOVE X ADD X \"new\" is the swap/rebind idiom.\n"
            "VARIANT AUTHORING (USD* entities only): ADD VARIANT[\"model\"] "
            "\"sedan\" creates the set (if needed) AND the variant. "
            "INSIDE VARIANT \"{model=sedan}\" retargets EVERY mutation clause "
            "of the statement into that variant, auto-creating the "
            "set/variant chain — so 'make one asset with variants from these "
            "assets' is: CREATE the shell prim, then one UPDATE per variant "
            "with INSIDE VARIANT \"{model=X}\" ADD REFERENCE \"x.usd\", then "
            "SET VARIANT[\"model\"] = \"X\" to pick the default. Also works "
            "for editing a variant WITHOUT switching to it (INSIDE VARIANT "
            "\"{look=red}\" SET VALUE = …). Nesting: concatenate pairs "
            "outermost-first (INSIDE VARIANT \"{model=sedan}{trim=sport}\") "
            "or create a set inside a variant (INSIDE VARIANT "
            "\"{model=sedan}\" ADD VARIANT[\"trim\"] \"sport\"). REMOVE "
            "VARIANT is not supported.\n"
            "FIND-THEN-MUTATE: cache a set with run_query (… AS \"n\"), then "
            "UPDATE <entity> IN RESULTSET \"n\" … mutates exactly those rows "
            "(stale rows = counted skips; an extra WHERE narrows further; prim "
            "rows project to their attributes for attribute statements and "
            "vice versa). Same world required — a USDPRIM set feeds UPDATE "
            "USD*; to edit or delete the AUTHORED specs behind composed prims "
            "use UPDATE/DELETE SDF* COMPOSING INTO RESULTSET \"n\" (or a "
            "\"/path\" / (\"/a\",\"/b\") list). DELETE SDFPRIM COMPOSING INTO "
            "… is the true 'delete everywhere' across all authoring layers.\n"
            "DELETE is authored-world only — deleting a composed USD* prim is "
            "a compile error; deactivate instead (UPDATE USDPRIM … SET ACTIVE "
            "= false) or delete the authored specs (DELETE SDFPRIM …).\n"
            "EXAMPLES:\n"
            "  UPDATE USDPRIM WHERE NAME LIKE \"debug\" SET ACTIVE = false\n"
            "  UPDATE USDPRIM WHERE VARIANT.SET CONTAINS \"lod\" "
            "SET VARIANT[\"lod\"] = \"proxy\"\n"
            "  UPDATE USDPRIM WHERE PATH = \"/Car\" INSIDE VARIANT "
            "\"{model=sedan}\" ADD REFERENCE \"sedan_car.usd\"\n"
            "  UPDATE USDPRIM WHERE KIND = \"component\" ADD "
            "VARIANT[\"damage\"] \"fine\" ADD VARIANT[\"damage\"] \"broken\"\n"
            "  UPDATE USDATTRIBUTE WHERE NAME = \"inputs:intensity\" AND "
            "VALUE.SCALAR > 1000 SET VALUE = 1000\n"
            "  UPDATE USDATTRIBUTE AT TIME 42 WHERE PATH = "
            "\"/World/door.rotateY\" SET VALUE = 90\n"
            "  UPDATE USDATTRIBUTE WHERE PATH = \"/World/door.rotateY\" "
            "SET VALUE = SAMPLES {1: 0.0, 12: 45.0, 24: 90.0}\n"
            "  UPDATE SDFATTRIBUTE IN LAYER \"overrides.usda\" WHERE NAME = "
            "\"radius\" SET VALUE = NULL\n"
            "  UPDATE SDFPRIM IN LAYERSTACK WHERE REFERENCE.IS_MISSING "
            "REMOVE REFERENCE\n"
            "  UPDATE SDFPRIM IN LAYERSTACK WHERE REFERENCE.ASSET LIKE "
            "\"chair_v1\" REMOVE REFERENCE ADD REFERENCE \"chair_v2.usd\"\n"
            "  UPDATE USDPRIM WHERE TYPE = \"Mesh\" ADD API "
            "\"PhysicsCollisionAPI\"\n"
            "  UPDATE USDRELATIONSHIP WHERE NAME = \"material:binding\" AND "
            "TARGET CONTAINS \"/Looks/Old\" REMOVE TARGET ADD TARGET "
            "\"/Looks/New\"\n"
            "  UPDATE USDRELATIONSHIP WHERE TARGET.IS_MISSING REMOVE TARGET\n"
            "  UPDATE USDPRIM WHERE TYPE = \"Mesh\" CREATE ATTRIBUTE "
            "\"primvars:mask\" TYPE \"float\" VALUE 0.0 INTERPOLATION "
            "\"constant\"\n"
            "  UPDATE USDPRIM WHERE KIND = \"component\" SET "
            "CUSTOMDATA[\"pipeline:reviewState\"] = \"approved\", "
            "CUSTOMDATA[\"pipeline:priority\"] = 3\n"
            "  UPDATE SDFATTRIBUTE IN LAYER \"asset.usda\" WHERE NAME = "
            "\"primvars:uv\" SET NAME = \"primvars:st\"\n"
            "  UPDATE SDFPRIM IN LAYER \"set.usda\" WHERE TYPE IS_A \"Light\" "
            "SET PARENT = \"/World/Lights\"\n"
            "  UPDATE LAYER WHERE IS_ROOT_LAYER SET FRAMES_PER_SECOND = 24\n"
            "  UPDATE LAYER WHERE IS_ROOT_LAYER AND SUBLAYER.IS_MISSING "
            "REMOVE SUBLAYER\n"
            "  CREATE USDPRIM \"/World/lights/key\" TYPE \"SphereLight\"\n"
            "  DELETE SDFPRIM IN LAYER \"scratch.usda\" WHERE PATH UNDER "
            "\"/World/debug\"\n"
        ...
        tools.push_back(JsValue(_Tool(
            ...
            "proxy', 'delete the temp primvars in this layer') instead of an "
            "explicit path list — the per-path edit tools (set_attributes, "
            "set_actives, …) stay best for a few known paths or a stored "
            "list_id. Targeting is run_query's FIND core (same entities, "
            "scopes, WHERE fields, RESULTSET chaining), so anything run_query "
            "can FIND this can mutate — including things no other edit tool "
            "reaches: authoring into a chosen layer (ON LAYER / IN LAYER), "
            "editing authored SDF* specs per layer, witness-gated arc removal "
            "(drop only the arcs the WHERE matched), variant selections, layer "
            "metadata (fps, default prim, sublayers), and cross-layer deletes "
            "(COMPOSING INTO). Returns a manifest — counts plus one row per "
